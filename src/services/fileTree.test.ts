...
function makeFileHandle(name: string): FileSystemFileHandle {
  return makeTestFileHandle({ name });
}
...
function makeDir(
  ...
  return makeTestDirectoryHandle({
    name,
    async *values() {
...
describe("buildFileTree", () => {
  ...
  it("builds paths relative to the root", async () => {
    ...
    const nestedDir = subDir.children[0];
    if (!nestedDir || nestedDir.kind !== "directory") {
      throw new Error("Expected a nested directory node");
    }
    const file = nestedDir.children[0];
