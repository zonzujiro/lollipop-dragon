...
describe("parseCriticMarkup — range anchors", () => {
  it("anchors a standalone comment to the requested quote occurrence", () => {
    const source =
      'Alpha target beta target. {>>fix: Compare this. @@ "target" @2<<}';
    const result = parseCriticMarkup(source);
    const [comment] = assignBlockIndices(result.comments, result.cleanMarkdown);

    expect(comment.text).toBe("Compare this.");
  ...
  it("supports escaped quotes and backslashes in anchor suffixes", () => {
    const source =
      'A "quoted" \\ path. {>>fix: Check this. @@ "\\"quoted\\" \\\\ path"<<}';
    const result = parseCriticMarkup(source);
    const [comment] = assignBlockIndices(result.comments, result.cleanMarkdown);

    expect(comment.anchor?.quote).toBe('"quoted" \\ path');
    expect(comment.anchor?.orphaned).toBe(false);
  });

  it("derives an anchor from wrapped highlight markup", () => {
    const source = "Read {==this exact text==}{>>note: Important.<<} again.";
    const result = parseCriticMarkup(source);
    const [comment] = assignBlockIndices(result.comments, result.cleanMarkdown);
  ...
  it("marks a missing quote as orphaned without losing the quote", () => {
    const source = 'No match. {>>fix: Restore this. @@ "missing"<<}';
    const result = parseCriticMarkup(source);
    const [comment] = assignBlockIndices(result.comments, result.cleanMarkdown);
  ...
  it("ignores anchor-like syntax inside fenced code blocks", () => {
    const source = '```md\n{>>note: code<<} @@ "code" @0\n```';

    expect(parseCriticMarkup(source).comments).toEqual([]);
  });

  it("attaches an anchored standalone comment after a fence to its source", () => {
    const source = [
      "```js",
      "run();",
      "run();",
      "```",
      '{>>question: Which call? @@ "run();" @2<<}',
      "",
      "Following paragraph.",
    ].join("\n");
    ...
    expect(parsed.cleanMarkdown).toContain("run();\nrun();");
  });
});
...
describe("parseCriticMarkup — cleanMarkdown", () => {
  ...
  it("keeps highlighted text, strips comment wrapper", () => {
    const { cleanMarkdown } = parseCriticMarkup(
      "before {==keep this==}{>>note<<} after",
    );
...
describe("parseCriticMarkup — cleanStart / cleanEnd", () => {
  it("cleanStart === cleanEnd for a standalone comment (zero-length)", () => {
    const { comments, cleanMarkdown } = parseCriticMarkup(
      "hello {>>note<<} world",
    );
    const [c] = comments;
  ...
  it("clean offsets are correct across multiple mixed comments", () => {
    ...
    expect(cleanMarkdown.slice(addition.cleanStart, addition.cleanEnd)).toBe(
      "add",
    );
  });
});
