...
describe("getBlockPositions — footnotes", () => {
  it("counts only rendered blocks and maps footnote definitions to one trailing index", () => {
    const md =
      "First paragraph with a ref.[^a]\n\nSecond paragraph.[^b]\n\n[^a]: First note text.\n\n[^b]: Second note text.";
    const positions = getBlockPositions(md);
    // two paragraphs render as blocks 0 and 1; both definitions share index 2
    // (the single rendered footnotes section)
    const indices = positions.map((position) => position.index);
    expect(indices).toEqual([0, 1, 2, 2]);
  });

  it("keeps paragraph indices stable when definitions sit mid-document", () => {
    const md =
      "Intro.[^a]\n\n[^a]: Mid-document note.\n\nOutro paragraph after the definition.";
    const comments = [
      makeComment(md.indexOf("Outro")), // inside the second rendered paragraph
    ];
    const [comment] = assignBlockIndices(comments, md);
    expect(comment.blockIndex).toBe(1);
  });

  it("maps a comment inside a footnote definition to the footnotes section index", () => {
    const md =
      "Only paragraph.[^a]\n\n[^a]: The footnote body being commented.";
    const comments = [makeComment(md.indexOf("footnote body"))];
    const [comment] = assignBlockIndices(comments, md);
    expect(comment.blockIndex).toBe(1);
  });

  it("ignores link reference definitions when counting blocks", () => {
    const md =
      "Uses a [link][ref].\n\n[ref]: https://example.com\n\nClosing paragraph.";
    const comments = [makeComment(md.indexOf("Closing"))];
    const [comment] = assignBlockIndices(comments, md);
    expect(comment.blockIndex).toBe(1);
  });
});
