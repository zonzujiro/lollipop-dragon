...
describe('ShareDialog — initial render', () => {
  it('shows the document label in the title', () => {
    ...
    expect(screen.getByRole('heading', { name: /Share "readme.md"/ })).toBeInTheDocument()
  })
...
describe('ShareDialog — share flow', () => {
  it('calls shareContent with selected TTL and shows link', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('textbox', { name: 'Shareable link' })).toHaveValue('https://example.com/#share=abc&key=xyz')
    })
  ...
  it('shows Encrypting… while loading', async () => {
    ...
    const shareContent = vi.fn().mockReturnValue(new Promise<string>((r) => { resolve = r }))
    useAppStore.setState({ shareContent })
    const user = userEvent.setup()
    render(<ShareDialog onClose={vi.fn()} />)
    await user.click(screen.getByRole('button', { name: 'Generate link' }))
    expect(screen.getByText('Encrypting…')).toBeInTheDocument()
    resolve!('https://example.com/#share=abc&key=xyz')
    await waitFor(() => {
