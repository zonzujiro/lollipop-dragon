...
function makeComment(overrides: Partial<Comment> = {}): Comment {
  return {
    id: '0', criticType: 'comment', type: 'fix', text: 'fix this',
    raw: '{>>fix: fix this<<}', rawStart: 0, rawEnd: 19,
...
describe('CommentCard — edit button', () => {
  it('shows Edit button for comment criticType when onEdit is provided', () => {
    render(<CommentCard comment={makeComment()} top={0} onClose={vi.fn()} onEdit={vi.fn()} />)
  ...
  it('shows Edit button for highlight criticType', () => {
    render(
      <CommentCard
        comment={makeComment({ criticType: 'highlight', highlightedText: 'some span' })}
  ...
  it('does not show Edit button for addition criticType', () => {
    render(
      <CommentCard comment={makeComment({ criticType: 'addition' })} top={0} onClose={vi.fn()} onEdit={vi.fn()} />,
    )
  ...
  it('calls onEdit with updated type and text on save', async () => {
    const user = userEvent.setup()
    const onEdit = vi.fn()
    render(<CommentCard comment={makeComment()} top={0} onClose={vi.fn()} onEdit={onEdit} />)
    await user.click(screen.getByRole('button', { name: 'Edit comment' }))
...
describe('CommentCard — delete button', () => {
  it('shows Delete button when onDelete is provided', () => {
    render(<CommentCard comment={makeComment()} top={0} onClose={vi.fn()} onDelete={vi.fn()} />)
  ...
  it('calls onDelete when confirmation is confirmed', async () => {
    ...
    render(<CommentCard comment={makeComment()} top={0} onClose={vi.fn()} onDelete={onDelete} />)
    await user.click(screen.getByRole('button', { name: 'Delete comment' }))
