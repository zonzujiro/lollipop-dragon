...
beforeEach(() => {
  useAppStore.setState({ peerName: null })
})
...
describe('PeerNamePrompt', () => {
  ...
  it('submit button is disabled when input is empty', () => {
    render(<PeerNamePrompt />)
    expect(screen.getByRole('button', { name: 'Start reviewing' })).toBeDisabled()
  })
  ...
  it('submit button is enabled when name is typed', async () => {
    const user = userEvent.setup()
    render(<PeerNamePrompt />)
    await user.type(screen.getByRole('textbox', { name: 'Your name' }), 'Alice')
    expect(screen.getByRole('button', { name: 'Start reviewing' })).toBeEnabled()
  })
  ...
  it('calls setPeerName with trimmed name on submit', async () => {
    ...
    const user = userEvent.setup()
    render(<PeerNamePrompt />)
    await user.type(screen.getByRole('textbox', { name: 'Your name' }), '  Bob  ')
  ...
  it('does not call setPeerName when name is empty', async () => {
    ...
    const user = userEvent.setup()
    render(<PeerNamePrompt />)
    // Try submitting without typing anything (button is disabled but test via form submit)
    await user.type(screen.getByRole('textbox', { name: 'Your name' }), '   ')
