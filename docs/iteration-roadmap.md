...
- Lazy-load a fine-grained Shiki highlighter only when a document contains a
  fenced code block
- Support the app's explicit language set: Bash, C, C++, CSS, Go, HTML, Java,
...
- Lazy-loads Mermaid, renders with strict security and HTML labels disabled,
  then sanitizes the returned SVG before it reaches the DOM
...
- Mermaid and Shiki are optional chunks that are not part of the application
  entry bundle; viewport-based Mermaid rendering remains a future optimization
