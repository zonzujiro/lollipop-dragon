...
3. **Comment** — select any span of text (or hover a block and press the `+`, or hit `C`). Pick a type with keys `1–6`, write, `⌘↵`. Watch the marker, rail card, and counts appear.
   ...
   - **Orphan grace** — if the agent rewrites anchored text, the comment keeps its quote and explains that the referenced text cannot be found while the comment remains saved, instead of mis-highlighting or vanishing (visible mid-agent-run on the clarify comment).
